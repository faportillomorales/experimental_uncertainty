        ...
        for i, jl in enumerate(jl_series):
            ...
            if len(jg_clean) > 0:
                ...
                for j, (jg_val, frictional_val, flow_pattern) in enumerate(zip(jg_sorted, frictional_sorted, flow_pattern_sorted)):
                    ...
                    symbol = pattern_data['symbol']
                    color = pattern_data['color']
                    ax.scatter(jg_val, frictional_val, c=color, marker=symbol, s=120, 
                             edgecolors='black', linewidth=1, zorder=2)
    ...
    try:
        ...
        for i, jl in enumerate(jl_series):
            ...
            if len(Re_g_clean) > 0:
                ...
                s_lines = symbol_line[i % len(symbol_line)]
                vec_s_lines.append(s_lines)
                # Ordenar dados por Re_g para conectar corretamente
                ...
                # Plotar linha conectando os pontos (todas pretas, estilo varia por série)
                ax.plot(Re_g_sorted, frictional_sorted, line_style, marker=s_lines, markersize=20, color='black', mfc='silver', linewidth=1.5, zorder=1)
                # ax.plot(Re_g_sorted, frictional_sorted, line_style, marker=s_lines, markersize=16, color='black', mfc='silver', linewidth=1.5, zorder=1)
                ...
                for j, (Re_g_val, frictional_val, flow_pattern) in enumerate(zip(Re_g_sorted, frictional_sorted, flow_pattern_sorted)):
                    ...
                    symbol = pattern_data['symbol']
                    color = pattern_data['color']
                    ax.scatter(Re_g_val, frictional_val, c=color, marker=symbol, s=120, 
                             edgecolors='black', linewidth=1, zorder=2)
                    
            
                print(f"Plotando série jL = {jl:.1f} m/s com {len(Re_g_clean)} pontos")
