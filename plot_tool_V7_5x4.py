...
# --- Estilo publicação: figura 6×4 pol.; tipografia 9–11 pt (boa em artigos 2 colunas) ---
PLOT_FIGSIZE = (5, 4)
PUB_FS_TICK = 10
PUB_FS_LABEL = 11
PUB_FS_LEGEND = 9
...
PUB_LW_SYSTEM = 2.0
PUB_SCATTER_S = 52
PUB_SCATTER_S_ORIENT = 66
PUB_SCATTER_EDGELW = 0.75
PUB_SCATTER_EDGELW_ORIENT = 0.9
PUB_ALPHA_LINE = 0.78
PUB_ALPHA_SCATTER = 0.86
PUB_GRID_ALPHA_RC = 0.38
PUB_LEGEND_MARKER_MS = 6.5
PUB_SCATTER_S_PANEL = 44
...
def setup_plot_style():
    """Estilo consistente para figuras 6×4 e publicação (PDF/PNG)."""
    plt.style.use('default')
    plt.rcParams.update({
        'figure.figsize': PLOT_FIGSIZE,
        'figure.dpi': 300,
        'font.size': PUB_FS_TICK,
        'font.family': 'serif',
        'axes.labelsize': PUB_FS_LABEL,
        'axes.titlesize': PUB_FS_LABEL,
        'xtick.labelsize': PUB_FS_TICK,
        'ytick.labelsize': PUB_FS_TICK,
        'legend.fontsize': PUB_FS_LEGEND,
        'lines.linewidth': PUB_LW_DATA,
        'lines.markersize': 5.0,
        'lines.markeredgewidth': PUB_SCATTER_EDGELW,
        'axes.linewidth': 0.85,
...
def finalize_plot_layout(fig=None, *, rect=(0.05, 0.08, 0.98, 0.87)):
    """Ajusta margens para legenda/topo e evita sobreposição (equivalente a tight_layout refinado)."""
    f = fig if fig is not None else plt.gcf()
    f.tight_layout(rect=rect, pad=0.65, h_pad=0.35, w_pad=0.35)

# PLOT_FIGSIZE definido acima (6×4 pol.).
...
# Lado de cada painel do mosaico (pol.); reduzido para coerência com figuras 6×4
MOSAIC_JL_JG_PANEL_IN = 2.85
...
def _add_theta_annotation_box(ax, theta, *, fontsize=PUB_FS_ANN):
...
def _selected_lowest_indices(selector_series, x_series, y_series, n=4):
    """Indices of the n valid points with the lowest selector value."""
    ...
    if not bool(valid.any()):
        return []
    return list(selector[valid].sort_values(kind='mergesort').head(n).index)
    ...
    for pattern in sorted(used, key=str):
        ...
        handles.append(
            plt.Line2D(
                ...
                markersize=PUB_LEGEND_MARKER_MS,
                markeredgecolor='black',
                markeredgewidth=PUB_SCATTER_EDGELW,
                label=pattern,
            )
        )
    return handles
...
def style_axes_re_g_log_x(
    ...
    *,
    ...
    ax.tick_params(axis='both', which='major', labelsize=PUB_FS_TICK, size=5.5)
    ax.tick_params(axis='both', which='minor', labelsize=PUB_FS_TICK, size=4.5)
...
def generate_alpha_vs_jg_plot(df, sheet_name, fluid_1, fluid_2, theta):
    ...
    try:
        ...
        lowest_jl_uncertainty_indices = _selected_lowest_indices(
            _measured_jl_for_legend_series(df_plot, jl_col),
        ...
        for i, jl_lbl in enumerate(jl_series):
            ...
            if len(jg_clean) > 0:
                ...
                for j, (jg_val, alpha_val, flow_pattern) in enumerate(zip(jg_sorted, alpha_sorted, flow_pattern_sorted)):
                    ...
                    ax.scatter(jg_val, alpha_val, c=color, marker=symbol, s=PUB_SCATTER_S,
                             edgecolors='black', linewidth=PUB_SCATTER_EDGELW, zorder=2)
        ...
        ax.set_xlabel(x_label, fontfamily='serif')
        ax.set_ylabel(y_label, fontfamily='serif')
        ...
        ax.tick_params(axis='both', which='major', labelsize=PUB_FS_TICK)
        _set_ticklabels_font_serif(ax)
        apply_subtle_gray_grid(ax)
        _format_axes_alpha_y_minor_half(ax, x_major_integers=True)
...
def generate_alpha_vs_beta_homogeneous_parity_plot(df, sheet_name, fluid_1, fluid_2, theta):
    ...
    try:
        ...
        ax.set_ylabel(r'$\alpha$ [-]', fontfamily='serif')
        ax.set_xlim(0.0, 1.0)
        ax.set_ylim(0.0, 1.0)
...
def _axis_tick_integer(x, pos):
    """Rótulos inteiros para eixo X (j_G, Re_sg, θ°, etc.)."""
    if not np.isfinite(x):
        return ''
    return f'{int(np.round(x))}'


def _format_axes_alpha_y_minor_half(ax, *, x_major_integers: bool):
    """
    Gráficos com α no eixo Y: ticks menores a cada 0,5 em Y; opcionalmente X com rótulos inteiros.
    """
    ax.yaxis.set_minor_locator(MultipleLocator(0.5))
    ax.yaxis.set_major_formatter(FuncFormatter(_axis_tick_decimal_linear))
    if x_major_integers:
        ax.xaxis.set_major_formatter(FuncFormatter(_axis_tick_integer))
    else:
...
def _decorate_jl_jg_matrix_subplot(
    ...
    *,
    ...
    if show_xlabel:
        ax.set_xlabel(r'$J_{g}$ [m/s]', fontfamily='serif')
    else:
        ax.set_xlabel('')
    if show_ylabel:
        ax.set_ylabel(r'$J_{l}$ [m/s]', fontfamily='serif')
    else:
        ax.set_ylabel('')
    ax.set_axisbelow(True)
    ax.tick_params(axis='both', which='major', labelsize=PUB_FS_SUBPANEL, size=5)
    ax.tick_params(axis='both', which='minor', labelsize=PUB_FS_TICK, size=4)
    _set_ticklabels_font_serif(ax)
    apply_subtle_gray_grid(ax)
    _add_theta_annotation_box(ax, theta)
...
def _finalize_jl_jg_matrix_axes(ax, *, legend_elements, theta):
    ...
    ax.set_ylabel(r'$J_{l}$ [m/s]', fontfamily='serif')
    ax.set_axisbelow(True)
    ax.tick_params(axis='both', which='major', labelsize=PUB_FS_TICK, size=5)
    ax.tick_params(axis='both', which='minor', labelsize=PUB_FS_TICK, size=4)
    _set_ticklabels_font_serif(ax)
    apply_subtle_gray_grid(ax)
    if legend_elements:
...
def _apply_jl_jg_flow_matrix_data_aspect(ax):
    """
    Mesma configuração que ``generate_alpha_vs_beta_homogeneous_parity_plot``:
    ``set_aspect('equal', adjustable='box')`` — 1 m/s em ``j_G`` corresponde à mesma
    extensão visual que 1 m/s em ``j_L``.
    """
    ax.set_aspect('equal', adjustable='box')
...
def generate_jl_vs_jg_flow_pattern_matrix_plot(df, sheet_name, fluid_1, fluid_2, theta):
    ...
    try:
        ...
        _apply_jl_jg_flow_matrix_data_aspect(ax_log)
        ...
        _apply_jl_jg_flow_matrix_data_aspect(ax_lin)
    ...
    try:
        ...
        def _draw_mosaic(scale_log: bool):
            ...
            for i, ax in enumerate(axes_list):
                ...
                _apply_jl_jg_flow_matrix_data_aspect(ax)
        ...
        print(
            f'Mosaico j_L vs j_G: {n} painéis (aspecto 1:1 j_G/j_L como na paridade α–β); log '
...
def generate_dpdzf_vs_jg_plot(df, sheet_name, fluid_1, fluid_2, theta):
    ...
    try:
        ...
        # Configurar eixos com fonte acadêmica
        ax.set_xlabel(x_label, fontfamily='serif')
        ax.set_ylabel(YLABEL_DP_DZ_F, fontfamily='serif')
        # Remover título conforme solicitado
...
def generate_dpdzt_vs_jg_plot(df, sheet_name, fluid_1, fluid_2, theta):
    ...
    try:
        ...
        x_label = r'$J_{g} [m/s]$'
        ax.set_xlabel(x_label, fontfamily='serif')
        ax.set_ylabel(YLABEL_DP_DZ_T, fontfamily='serif')
...
def generate_alpha_vs_Reg_plot(df, sheet_name, fluid_1, fluid_2, theta):
    ...
    try:
        ...
        flow_pattern_symbols = get_flow_pattern_symbols()
        alpha_unc_col = _find_point_uncertainty_column(df_plot, col_mapping, 'U_alpha')
        lowest_resl_uncertainty_indices = _selected_lowest_indices(
            _one_col_series(df_plot, 'Re_sl_group'),
        ...
        ax.set_xlabel(r'$Re_{sg}$ [-]', fontfamily='serif')
        ax.set_ylabel(r'$\alpha$ [-]', fontfamily='serif')
...
def _generate_orientation_plot_for_quantity(
    ...
    for re_l in unique_re_sl:
        ...
        # Mesma proporção de figura que os demais plots (6×4 pol.)
        fig, ax = plt.subplots(figsize=PLOT_FIGSIZE)
                ...
                for sys in systems:
                    ...
                    for _, row in subset.iterrows():
                        ...
                        ax.scatter(
                            ...
                            c=pattern_color,
                            marker=symbol,
                            s=PUB_SCATTER_S_ORIENT,
                            edgecolors=color,
                            linewidth=PUB_SCATTER_EDGELW_ORIENT,
                            zorder=2,
                            alpha=PUB_ALPHA_SCATTER,
                        )
        ...
        ax.set_xlabel('Pipe inclination (θ) [°]', fontfamily='serif')
        ax.set_ylabel(y_label, fontfamily='serif')
        ax.set_axisbelow(True)
        ax.minorticks_on()
        ...
        finalize_plot_layout(fig, rect=(0.05, 0.08, 0.98, 0.88))
        ...
        with open(os.devnull, 'w') as devnull:
            with redirect_stderr(devnull):
                fig.savefig(pdf_file, format='pdf', **PLOT_SAVEFIG_KWARGS)
                fig.savefig(png_file, format='png', **PLOT_SAVEFIG_KWARGS)
        print(f"Plot PDF salvo: {pdf_file}")
        print(f"Plot PNG salvo: {png_file}")
